...
class EvolutionProvider:
    ...
    async def send_audio(self, to: str, audio_url: str) -> OutboundResult:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{base}/message/sendMedia/{instance}",
                json={"number": to, "mediatype": "audio", "media": audio_url},
                headers=self._headers(),
            )
        if r.status_code >= 400:
            raise ProviderError(f"Evolution send_audio {r.status_code}: {r.text}")
        data = r.json() if r.content else {}
        return OutboundResult(wa_message_id=self._extract_wa_id(data), raw=data)
    ...
    async def send_video(self, to: str, video_url: str, caption: str = "") -> OutboundResult:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{base}/message/sendMedia/{instance}",
                json={"number": to, "mediatype": "video", "media": video_url, "caption": caption},
                headers=self._headers(),
            )
        if r.status_code >= 400:
            raise ProviderError(f"Evolution send_video {r.status_code}: {r.text}")
        data = r.json() if r.content else {}
        return OutboundResult(wa_message_id=self._extract_wa_id(data), raw=data)
    ...
    async def send_document(self, to: str, document_url: str, filename: str = "", caption: str = "") -> OutboundResult:
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.post(
                f"{base}/message/sendMedia/{instance}",
                json={"number": to, "mediatype": "document", "media": document_url, "fileName": filename, "caption": caption},
                headers=self._headers(),
            )
        if r.status_code >= 400:
            raise ProviderError(f"Evolution send_document {r.status_code}: {r.text}")
        data = r.json() if r.content else {}
        return OutboundResult(wa_message_id=self._extract_wa_id(data), raw=data)
