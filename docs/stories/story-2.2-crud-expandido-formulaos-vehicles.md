...
- [x] **AC1 (Persistência Completa de Atributos)**: Os endpoints `POST /api/vehicles` e `PATCH /api/vehicles/{id}` devem receber, validar e persistir todos os campos da tabela `formulaos_vehicles`:
...
- [x] **AC2 (Isolamento por Perfil - RBAC)**:
...
- [x] **AC3 (Sincronização Bidirecional e Triggers)**:
...
- [x] **AC4 (Consulta e Retorno Detalhado)**:
...
- [x] **AC5 (Compatibilidade com Vitrine Pública)**:
...
- [x] **Task 1 (Rotas de Veículos)**: Expandir `_FIELDS`, `_REQUIRED` e `_PATCHABLE` em `backend/routes/vehicles.py` para incluir todos os novos campos.
- [x] **Task 2 (Mapeamento de Tipos e Serialização)**: Tratar tipos especiais no SQLite e PostgreSQL:
...
- [x] **Task 3 (Adaptação da Consulta GET)**: Garantir que `store_name` e todos os campos sejam retornados no payload para a tabela e o modal do frontend.
- [x] **Task 4 (Ajustes no Portal Público)**: Garantir que `backend/routes/public.py` utilize os novos campos para alimentar os cards da vitrine e a página de detalhes com a galeria completa.
- [x] **Task 5 (Testes Automatizados)**: Implementar `tests/test_vehicles_crud_expanded.py` cobrindo cenários com e sem loja, validação de payload e perfil lojista vs gestor.
- [x] **Task 6 (Quality Gate)**: Rodar `pytest tests/test_vehicles_crud_expanded.py` e `ruff check backend tests`.
