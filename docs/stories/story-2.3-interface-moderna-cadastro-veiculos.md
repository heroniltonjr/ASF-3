...
- [ ] **AC1 (Dropzone e Galeria de Fotos - Two-Phase Upload)**:
...
- [ ] **AC2 (Classificação Veicular Estruturada)**:
...
- [ ] **AC3 (Ano, KM e Valores com Máscaras)**:
...
- [ ] **AC4 (Badges Comerciais - Toggles)**:
  - Cards ou switches compactos para os booleanos:
    ...
    - `sold`: "Vendido"
- [ ] **AC5 (Seletor de Opcionais por Chips - `item_list`)**:
...
- [ ] **AC6 (Diferenciação por Perfil - RBAC)**:
...
- [ ] **Task 1 (Componentes de Estilo)**: Adicionar em `styles.css` os estilos para o novo modal ampliado: dropzone de fotos, grid de miniaturas com badges, chips de opcionais (`.chip-list`, `.chip`), switches toggle e layout em seções/grid.
- [ ] **Task 2 (Componente Dropzone no JavaScript)**: Em `app.js`, implementar o componente de upload de fotos desacoplado com pré-visualização instantânea e reordenação drag & drop.
- [ ] **Task 3 (Reformulação do Modal de Veículo)**: Atualizar `openVehicleModal()` em `app.js` para renderizar as novas seções: Galeria, Classificação, Ano/KM/Mecânica, Valores, Badges, Opcionais e Observações.
- [ ] **Task 4 (Integração de Edição)**: Garantir que a edição de veículos (`data-vehicle-action="edit"`) preencha a galeria existente a partir do array `pictures` e monte os chips selecionados em `item_list`.
- [ ] **Task 5 (Cache Busting)**: Atualizar `index.html` para `app.js?v=1.7` e `styles.css?v=1.7`.
- [ ] **Task 6 (Validação e Quality Gate)**: Testar fluxos de cadastro e edição nos perfis `gestor` e `lojista`, rodar a suíte completa de testes com `pytest`.
